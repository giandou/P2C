...
char *gen_expression( listaStringhe *expr ) {
    char *expression;
...
//assegna a expression il primo valore della lista Exp.
	...
	//expression = (char *) malloc(sizeof(char) * strlen(expr->stringa);
...
//concatena tutti i successivi elementi della lista.
	...
	expression = (char*) realloc(expression, sizeof(char) * (strlen(expr->stringa) + strlen(expression) +1));
        strcat( expression, expr->stringa );
        expr = expr->next;
    }
    return expression;
}
...
/*
void print_expression( FILE * f_ptr, listaStringhe *Expr ) {
    listaStringhe * Exp = Expr;
    
    printf("%s",strlen(Exp->stringa));
    char * expression = (char *)malloc(sizeof(char) * (strlen(Exp->stringa)+1));
...
//get_listaStringhe( Exp, "ESPR" );
    ...
    while ( Exp != NULL ) {
	expression = (char *)realloc(expression,sizeof(char)*(strlen(expression)+strlen(Exp->stringa)+1));
        strcat( expression, Exp->stringa );
        Exp = Exp->next;
    }
    //strcat(expression, "\0");
